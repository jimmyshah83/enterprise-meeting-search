 ...
 *
 * When no results are found it generates a diagnostic summary explaining
 * likely reasons so that callers can give actionable feedback to the user.
 */
...
/**
 * Produces a human-readable diagnostic summary when all domains return no
 * results.  The summary is tailored based on which domains timed out or
 * errored vs. which simply returned empty result sets.
 */
function buildDiagnosticSummary(
  ...
  if (timedOutDomains.length > 0) {
    reasons.push(
      `The following domains did not respond in time: ${timedOutDomains.join(', ')}.`,
    );
    suggestions.push(
      'Try again later, or increase the domain timeout via the domainTimeoutMs option.',
    );
  }

  if (erroredDomains.length > 0) {
    reasons.push(
      `The following domains returned errors: ${erroredDomains.join(', ')}.`,
    );
  ...
  if (request.keywords && request.keywords.length > 0) {
    reasons.push(
      `No content matched the keywords: "${request.keywords.join('", "')}".`,
    );
    suggestions.push(
      'Try broader or alternative keywords.',
      'Verify that the correct date range is set.',
    );
  }
  ...
  if (request.dateRange) {
    ...
    reasons.push(
      `No meetings were found in the date range ${start.toISOString()} – ${end.toISOString()}.`,
    );
  ...
  if (request.organizer) {
    reasons.push(
      `No meetings were found organized by: ${request.organizer}.`,
    );
  ...
  if (reasons.length === 0) {
    ...
    suggestions.push(
      'Try relaxing the search filters.',
      'Ensure you have permissions to access the relevant data sources.',
    );
  }
...
export async function search(
  ...
  if (results.length === 0) {
    return {
      results: [],
      diagnostics: buildDiagnosticSummary(request, domainStatuses),
      totalResults: 0,
      searchDurationMs,
    };
  }
